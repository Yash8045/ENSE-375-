...
public class DateTime 
{
	...
	*/
	...
	{
		int diff=0;
		Boolean dateCheck = Date.equal(d1.date, d2.date);
		//Todo: add your code here
		if ((dateCheck == false))
		{
			//MoreThanOneDayException();
		}
		else if (dateCheck == true)
		{
			Time12 time1 = new Time12(d1.getTime().getHours(), d1.getTime().getMinutes(), d1.getTime().getAM_or_PM());
			Time12 time2 = new Time12(d2.getTime().getHours(), d2.getTime().getMinutes(), d2.getTime().getAM_or_PM());
	
			diff = Time24.subtract(time1.toTime24(), time2.toTime24());
		}
